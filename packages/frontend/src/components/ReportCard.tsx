...
export default function ReportCard({ report }: Props) {
  ...
  return (
    <Link
      ...
      className="block border border-gray-800 rounded-lg p-5 hover:border-amber-500/50 transition-colors"
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-mono text-gray-500">{report.reportKey}</span>
      ...
      {synthesis?.outlook && (
        <p className="text-sm text-gray-300 line-clamp-3">{synthesis.outlook}</p>
      )}
